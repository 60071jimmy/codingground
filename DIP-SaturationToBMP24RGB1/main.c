/*	數位影像處理程式
	Develop by Jimmy
	本程式讀取BMP圖檔，並將讀取之RGB圖像資料轉換至HSV色彩空間，再將HSV色彩空間之飽和度Saturation取出建立Saturation.BMP圖檔
	像素之飽和度(Saturation)愈大，對應至Saturation.BMP圖像之亮度愈亮
 */
//-----引入標頭檔-----
	...
	*/
	...
	{																	//	進入else敘述 
		...
		{																//	進入else敘述 
			...
			//ViewBMPImage("Saturation");									//	呼叫ViewBMPImage檢視圖檔
			//system("pause");											//	暫停程式 
			return 0;													//	傳回0 
		}																//	結束else敘述 
	}																	//	結束else敘述 
	...
	{																	//	進入for迴圈
		...
		{																//	進入for迴圈
			...
			long double H1 = acos(0.5 * ((Red - Green) + (Red - Blue)) /
							 ...
							 (Red - Blue) * (Green - Blue)))) * (360.0/(2 * M_PI));
			if( Max == Min )											//	若Max=Min
			{															//	進入if敘述
			...
			#ifdef DebugMode											//	若有定義為DebugMode(除錯模式)
				printf("\n轉換結果：\n原始像素\t轉換後像素\n");
				printf("R=%d\tH=",InputData[LoopNumber1 * xsize + LoopNumber2].R);
				ShowLongDouble(OutputData[LoopNumber1 * xsize + LoopNumber2].H);
				printf("\nG=%d\tS=",InputData[LoopNumber1 * xsize + LoopNumber2].G);
				ShowLongDouble(OutputData[LoopNumber1 * xsize + LoopNumber2].S);
				printf("\nB=%d\tV=",InputData[LoopNumber1 * xsize + LoopNumber2].B);
