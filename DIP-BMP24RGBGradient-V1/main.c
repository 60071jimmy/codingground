...
/*	BMP24RGBGradient副程式用於計算BMP24RGB型態圖像資料之梯度
	運算方法為
		...
		Direction = atan( Gy / Gx ) * (180 / PI)	if Gx != 0
							 90						if (Gx = 0) and (Gy > 0)
							-90						if (Gx = 0) and (Gy < 0)
							0(Nan?)					if (Gx = 0) and (Gy = 0)	當Gx為0且Gy亦為0時，梯度方向無法定義
		
		本副程式將圖像像素之R、G、B成分分別處理
...
{																		//	進入主程式 
	...
	if( (BMPImage1.XSIZE == -1) || (BMPImage1.YSIZE == -1) )			//	若XSIZE或YSIZE為-1(代表讀取檔案失敗)	
	{																	//	進入if敘述 
	...
	{																	//	進入else敘述 
		...
		printf("輸入影像大小(Byte)：%d\n",(size_t)BMPImage1.XSIZE * BMPImage1.YSIZE * 3);
		//	顯示輸入影像大小數值(Byte) 
		BMPImage1.FILLINGBYTE = BmpFillingByteCalc(BMPImage1.XSIZE);	//	呼叫BmpFillingByteCalc副程式計算填充之位元組數量
		BMPImage1.IMAGE_DATA = (unsigned char*)malloc((BMPImage1.XSIZE * 3 + BMPImage1.FILLINGBYTE) * BMPImage1.YSIZE * sizeof(unsigned char));
		//	計算並建立影像大小空間 
		if (BMPImage1.IMAGE_DATA == NULL) 								//	若建立影像空間失敗 
		{																//	進入if敘述 
		...
		{																//	進入else敘述 
			...
			for(loop_num=0;loop_num<((BMPImage1.XSIZE * 3 + BMPImage1.FILLINGBYTE) * BMPImage1.YSIZE);loop_num++)
			//	以for迴圈初始化每個像素 
			{															//	進入for迴圈 
				BMPImage1.IMAGE_DATA[loop_num]=255;						//	填入預設像素色彩數值 
			}															//	結束for迴圈 
			BmpRead(BMPImage1.IMAGE_DATA, BMPImage1.XSIZE, BMPImage1.YSIZE, BMPImage1.FILENAME);
			//	讀取圖檔資料
...
{																		//	進入BMP24RGBGradient(BMP圖片資料梯度計算)副程式
	...
	{																	//	進入for迴圈
		...
		{																//	進入else敘述
			...
			OutputData[loop_num].R = (unsigned char)Magnitude;			//	將計算結果填入像素資料
			...
			OutputData[loop_num].G = (unsigned char)Magnitude;			//	將計算結果填入像素資料
			...
			OutputData[loop_num].B = (unsigned char)Magnitude;			//	將計算結果填入像素資料
