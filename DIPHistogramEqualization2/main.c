 ...
 */
/*	FileExistCheck副程式用於檢查指定檔案是否存在
	副程式輸入為欲檢查是否存在之檔案路徑
	副程式輸出為布林變數，若檔案存在為true；若檔案不存在為false
 */
bool FileExistCheck(char *);											//	宣告FileExistCheck(檔案存在檢查)副程式
/*	File_delete副程式用於刪除指定檔案
	副程式輸入為欲刪除之檔案路徑
	副程式輸出為void
 */
	...
	*/
	...
	{																	//	進入else敘述 
		...
		{																//	進入else敘述 
			...
			sprintf(OutputFileName,"%s%s",filename,"-RGBGHE2");			//	建立輸出檔名字串(OutputFileName)
...
{																		//	進入BmpWrite(BMP圖檔寫入)副程式 
	...
	if(FileExistCheck(fname_bmp) == true)								//	若欲寫入之檔案存在
	{																	//	進入if敘述
		printf("檔案已存在，將覆寫舊檔案\n");							//	顯示"檔案已存在，將覆寫舊檔案"並換行
		File_delete(fname_bmp);											//	刪除舊檔案
	}																	//	結束if敘述
...
{																		//	進入RGBHistogramEqualization(RGB灰階影像直方圖等化)副程式
	...
	for(loop_num = 0; loop_num <(xsize * ysize); loop_num++)			//	以for迴圈依序初始化OutputData圖像像素
	{																	//	進入for迴圈
		OutputData[loop_num].R = 255;									//	設定像素初始值
		OutputData[loop_num].G = 255;									//	設定像素初始值
		OutputData[loop_num].B = 255;									//	設定像素初始值
	}																	//	結束for迴圈
	long long int CountPixel[256];										//	宣告CountPixel陣列統計各像素值出現次數
	long long int HEPixelNum = 0;										//	宣告HEPixelNum變數，記錄直方圖等化總像素
	//***以for迴圈依序初始化CountPixel統計值為0***
	for(loop_num = 0; loop_num <(xsize * ysize); loop_num++)			//	以for迴圈依序初始化OutputData圖像像素
	{																	//	進入for迴圈
		if( (image[loop_num].R != 255) || (image[loop_num].G != 255) || (image[loop_num].B != 255) )
		//	若像素之R、G、B值皆為255(全白)則不計入
		{																//	進入if敘述
			HEPixelNum = HEPixelNum + 1;								//	遞增HEPixelNum變數
		}																//	結束if敘述
	}																	//	結束for迴圈
	...
	unsigned long long int PDF[255],CDF[255];							//	宣告PDF與CDF陣列，計算個別像素值機率與累計機率
	for(loop_num = 0; loop_num < 255; loop_num++)						//	以for迴圈依序計算PDF
	{																	//	進入for迴圈
		PDF[loop_num] = CountPixel[loop_num];							//	計算像素值PDF
	}																	//	結束for迴圈
	CDF[0] = PDF[0];													//	填入像素值為0的CDF
	for(loop_num = 1; loop_num < 255; loop_num++)						//	以for迴圈依序計算CDF
	{																	//	進入for迴圈
	...
	for(loop_num = 0; loop_num < 255; loop_num++)						//	以for迴圈依序HistogramEqualization後像素值
	{																	//	進入for迴圈
		ResultPixel[loop_num] = CDF[loop_num] * 255 / HEPixelNum;		//	計算HistogramEqualization後像素值
	}																	//	結束for迴圈
	ResultPixel[255] = 255;												//	給定白色區塊數值
