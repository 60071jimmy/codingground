 ...
 */
char BmpRead(unsigned char*,const int,const int, const char *);			//	宣告BmpRead(BMP圖檔讀取)副程式
 ...
 */
BMPIMAGE BmpFileRead(const char *);										//	宣告BmpFileRead副程式
...
/*	InitialIMGArrayTest副程式用於測試InitialIMGArray副程式
	本測試副程式之輸入、輸出皆為void以觀察使用副程式所需宣告與語法
	副程式輸入為void
	副程式輸出為void
 */
...
{																		//	進入主程式 
	...
	BMPImage1 = BmpFileRead(BMPImage1.FILENAME);						//	呼叫BmpFileRead副程式讀取BMP圖檔
	...
	//ViewBMPImage("BMPGradient");										//	呼叫ViewBMPImage檢視圖檔
			
	//system("pause");													//	暫停程式 
	return 0;															//	傳回0 
}																		//	結束主程式 
//----BMP圖檔大小(Byte)讀取副程式----
unsigned long BmpReadFilesize(const char *fname_bmp)					//	BmpReadFilesize副程式
...
//----BMP圖檔xsize(寬度)讀取副程式----
unsigned long BmpReadXSize(const char *fname_bmp)						//	BMP圖檔xsize(寬度)讀取副程式
...
//----BMP圖檔ysize(高度)讀取副程式----
unsigned long BmpReadYSize(const char *fname_bmp)						//	BMP圖檔ysize(高度)讀取副程式
...
//----BMP圖檔讀取副程式---- 
char BmpRead(unsigned char *image,const int xsize,const int ysize, const char *fname_bmp)
{																		//	進入BMP圖檔讀取副程式
...
BMPIMAGE BmpFileRead(const char *filename)								//	BmpFileRead副程式
{																		//	進入BmpFileRead副程式
	...
	FILE *fp;															//	宣告檔案指標fp區域變數
	fp = fopen(filename, "rb");											//	以rb(二進位讀取)模式開啟檔案
	if (fp == NULL)														//	若開啟檔案失敗 
	{																	//	進入if敘述 
		printf("讀取檔案失敗！\n");										//	顯示錯誤訊息 
		return OutputData;												//	回傳OutputData，並結束副程式
	}																	//	結束if敘述	 
	stpcpy(OutputData.FILENAME, filename);								//	填入檔案路徑
	OutputData.XSIZE = (unsigned int)BmpReadXSize(OutputData.FILENAME);	//	讀取輸入BMP圖檔寬度 
	OutputData.YSIZE = (unsigned int)BmpReadYSize(OutputData.FILENAME);	//	讀取輸入BMP圖檔高度 
	...
	{																	//	進入else敘述 
		...
		{																//	進入else敘述 
			...
			BmpRead(OutputData.IMAGE_DATA, OutputData.XSIZE, OutputData.YSIZE, OutputData.FILENAME);
			//	讀取圖檔資料
		}																//	結束else敘述
	}																	//	結束else敘述
